...
export async function GET(
  ...
  const home = new URL("/", process.env.APP_URL || "http://localhost:3000");
  try {
    ...
    jar.set("outpost_oauth", "", { maxAge: 0, path: "/api/oauth" });
    ...
    const tokenParams = new URLSearchParams({
      ...
      code,
      redirect_uri: new URL(`/api/oauth/${platform}/callback`, home).toString(),
    });
