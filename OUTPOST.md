...
3. Merge the values from `.env.outpost.example` into your existing `.env.local` and fill in the Supabase URL, anon/publishable key, service-role key, and database transaction-pooler URL. Use the pooler suitable for Vercel (typically port 6543). Prepared statements are disabled in the PostgreSQL client.
...
1. Use the existing nandann.com Vercel project, with its Root Directory at the repository root. The website and admin console deploy together.
2. Add the variables from `.env.outpost.example` in Vercel's **Production** environment. Set `APP_URL` to the stable production domain. Use a separate database/apps for Preview deployments if you enable live functionality there.
3. Deploy production. `vercel.json` runs `/admin/api/cron` every minute. Vercel supplies `CRON_SECRET` as the bearer token. Verify the cron exists in the Vercel dashboard. Cron is not a guarantee of second-exact posting.
...
The console is integrated into the website's Next.js application at `app/admin`, with supporting modules in `lib/outpost` and `components/outpost`. Keep the existing Vercel project and its repository-root setting. There is no separate admin deployment or proxy. The console URL is https://nandann.com/admin (locally http://localhost:3000/admin). Set `APP_URL` to the actual browser origin, without `/admin`, including the port locally. Add the settings from `.env.outpost.example` to the existing Vercel project and apply all four SQL migrations in `db/outpost` before connecting accounts. The website feed stays at `/api/social/articles`. Admin APIs and OAuth callbacks use `/admin/api/...`. The root `vercel.json` schedules the authenticated publishing endpoint every minute.
