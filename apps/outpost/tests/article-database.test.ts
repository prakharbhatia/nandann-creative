...
vi.mock("../lib/db", () => ({ db: () => shared.sql }));
vi.mock("../lib/providers", async (importOriginal) => ({
  ...(await importOriginal<typeof import("../lib/providers")>()),
  publish: vi.fn(),
  publishComment: vi.fn(),
}));
import { publish, publishComment, ProviderError } from "../lib/providers";
import { runCommentQueue } from "../lib/comment-queue";
import { runQueue } from "../lib/queue";
import { enqueueArticle } from "../lib/article-queue";
import { syncArticleFeed } from "../lib/article-feed";
const pg = new PGlite();
...
beforeAll(async () => {
  ...
  for (const path of [
    "db/schema.sql",
    "db/002_article_queue.sql",
    "db/003_article_feed.sql",
    "db/004_admin_console.sql",
  ])
