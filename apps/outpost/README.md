...
1. Create a Supabase project and run `db/schema.sql` in its SQL editor. This also creates the private `post-media` bucket. Run the schema as the project administrator.
...
3. Copy `.env.example` to `.env.local` and fill in the Supabase URL, anon/publishable key, service-role key, and database transaction-pooler URL. Use the pooler suitable for Vercel (typically port 6543). Prepared statements are disabled in the PostgreSQL client.
...
```text
https://YOUR-DOMAIN/api/oauth/linkedin/callback
https://YOUR-DOMAIN/api/oauth/linkedin-page/callback
```
...
```text
https://YOUR-DOMAIN/api/oauth/x/callback
```
...
1. Import this directory as a Next.js project (push it to your own Git repository and import, or run `npx vercel` from this directory).
...
3. Deploy production. `vercel.json` runs `/api/cron` every minute. Vercel supplies `CRON_SECRET` as the bearer token. Verify the cron exists in the Vercel dashboard. Cron is not a guarantee of second-exact posting.
...
The server-to-server article endpoint books one article per day at 6 PM India time across the connected accounts. Apply `db/002_article_queue.sql`, configure the website key, and connect the website publication event. See [the API contract and integration guide](docs/website-api.md). It deduplicates retries by stable article ID or canonical URL and reserves dates transactionally.
...
Apply `db/004_admin_console.sql` after the previous migrations. **Automation** now stores article time, publishing timezone, and two daily image-post times in PostgreSQL. Defaults are 10:00 and 16:00 for image posts, and 18:00 for articles, all in Asia/Kolkata. Changes apply only to new bookings. Disabling automatic booking does not cancel posts already scheduled. Existing bookings remain visible in Publishing and can be canceled there.
...
This admin application lives at `apps/outpost` in https://github.com/prakharbhatia/nandann-creative. Create a separate Vercel project from that repository and set **Root Directory** to `apps/outpost`; the existing website project keeps its current root. Configure secrets only in the admin project's Vercel environment. The website article API stays in the root website application.
