...
Every newly published website article calls `POST /admin/api/v1/articles` from the website's server. Do not call this from a browser or expose the key in public JavaScript.
...
- Apply `db/outpost/002_article_queue.sql` after `db/outpost/schema.sql`.
