...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const deleteResponse = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/deleteWebhook?drop_pending_updates=true`);
    ...
    // Re-setup webhook
    const setupResponse = await fetch(`${protocol}://${host}/api/chat/setup-webhook`);
    ...
    return res.status(200).json({
      ...
      results: processed,
      webhookReconfigured: setupData.success,
    });
  } catch (error) {
